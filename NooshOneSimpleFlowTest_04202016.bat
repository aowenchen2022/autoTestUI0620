...
if "%%A" == "sdr" set DOMAIN=-Dselenium.domain=sdr
if "%%A" == "nooshentry" set COMPANYNAME=-Dselenium.companyname=nooshentry
)
set DOMAIN=-Dselenium.domain=scd
set BROWSER=-Dselenium.browser=Chrome
set BUYERBROWSER=-Dselenium.browser.buyer=Chrome
mvn clean integration-test %BROWSER% %DEBUGINFO% %BUYERBROWSER% %DOMAIN% %COMPANYNAME% -DtestngFile=testngNooshOneSimpleFlow.xml
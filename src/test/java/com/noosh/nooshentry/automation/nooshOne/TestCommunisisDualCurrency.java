...
public class TestCommunisisDualCurrency extends BaseSeleniumTest {
	...
	public void testCreateRFE(String specName, String Quantity)
		...
		//brochurePopup.setComplationDate(); 
		WebElement supplierInput = driver.findElement(By.className("contactSelectorContainer")).findElement(By.tagName("input"));
		testSelectContact("Coenen", supplierInput);
		Thread.sleep(1000);
		for (String handle : driver.getWindowHandles()) {
			driver.switchTo().window(handle);
		}
	...
	public void testSupplierEstCreate() throws InterruptedException {
		...
		Thread.sleep(1000);
		String price = nobrs.get(19).findElement(By.tagName("input")).getAttribute("value");
		//System.out.print("test is:"+price);
		switch(locale){
		case "en_US":
			Assert.assertEquals(price, "6.30000");		
			break;
		case "en_EU":
			Assert.assertEquals(price, "6,30000");	
			break;
	...
	public void testQuoteCreate() throws Exception {
		...
		String grandTotal = driver.findElement(By.name("quote.grandTotal")).getAttribute("value");
		...
		new Select(driver.findElement(By.id("@quote.quoteItem1.uom_str"))).selectByValue("Unit");
		Thread.sleep(1000);
		//Assert.assertEquals(grandTotal, "141,451.95");
		driver.findElement(By.id("send_quote")).click();
		Thread.sleep(1000);
		driver.findElement(By.id("quoteCreate_body")).findElement(By.className("button")).click();
		Thread.sleep(1000);
...
//		driver.findElement(By.id("orderItem1.exValue")).sendKeys("100");
		//List<WebElement> nobrs = driver.findElement(By.id("span_itemIndex1_7")).findElements(By.tagName("nobr"));
		//WebElement ChangePrice = nobrs.get(2).findElement(By.tagName("input"));
		WebElement ChangePrice = driver.findElement(By.id("orderItem1.exValue"));
        JavascriptExecutor executor = (JavascriptExecutor) driver;   
        executor.executeScript("arguments[0].value ='"+"100"+"';",ChangePrice);
    	Thread.sleep(1000);
        ChangePrice.sendKeys("{Tab}");
		List<WebElement> commentInputs = driver.findElements(By.tagName("textarea"));
		commentInputs.get(1).sendKeys("Change order comments");
		Thread.sleep(1000);
	...
	public void testCloseChangeOrderCreate(String fileForProject11) throws InterruptedException, AWTException {
		//HomePage homePage = new HomePage(driver);

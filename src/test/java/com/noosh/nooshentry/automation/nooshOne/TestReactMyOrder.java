...
@Test
public class TestReactMyOrder extends BaseSeleniumTest {
	...
	public void testSearchFindProject(String specName, String Quantity)
		...
		search.get(1).click();
		Thread.sleep(500);
		search.get(1).findElement(By.tagName("input")).sendKeys("5113705");
		Thread.sleep(1000);
	...
	public void testCreateBuyOrder() throws Exception {
		...
		Thread.sleep(1500);
		builder.moveToElement(orderLnks.get(0)).moveToElement(driver.findElement(By.className("glb-flyout")).findElement(By.linkText("Create..."))).click().perform();
		Thread.sleep(2000);
		for (int i = 0; i < 2; i++) { 
	...
	//	commonUtils.testChooseCalendar();
		...
		testSelectContact("uhg", supplierInput);
		Thread.sleep(1000);
		new Select(driver.findElement(By.id("@order.SUPPLIER_SELECTION_REASON_str"))).selectByIndex(1);
		
		
		driver.findElement(By.id("order.completionDate")).sendKeys("9/29/20");
		Thread.sleep(1000);
		driver.findElement(By.id("orderItem1.unitPrice")).sendKeys("1000");
		new Select(driver.findElement(By.id("@order.SOW_NO_str"))).selectByIndex(1);

		Thread.sleep(1000);
		driver.findElement(By.id("@order.PAY_INV_VIA_DEPOT_bool")).click();
		driver.findElement(By.id("orderItem1.quantity")).sendKeys("100");
	...
	/*	List<WebElement> orderLnks = driver.findElements(By.className("fa-shopping-cart"));
		...
		for (int i = 0; i < 100; i++) { 
		System.out.print("This is the" + i + "times");
		driver.findElement(By.id("leftnav_order_create")).click();
		Thread.sleep(1000); 
		driver.findElement(By.id("specRefId")).click();
		Thread.sleep(500); 
		driver.findElement(By.id("create_quick_order")).click();
		...
		driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
		try {
		if(driver.findElement(By.id("leftnav_order_create")).isDisplayed() == false){
			captureScreenshot(null);
			System.out.print("Failed at the" + i + "times");
		}
...
/*
	...
	public void testSelectProjFromRecent() throws Exception {
		...
		Thread.sleep(1000);
		driver.switchTo().frame("rpmenu_iframe");
    ...
    public static void captureScreenshot(String picName) {
        // take the screenshot
    	SimpleDateFormat random = new SimpleDateFormat("MMddyyhhmmss");
        ...
        if (picName == null) {
            picName = random.format(new Date()) + ".png";
        } else {
            picName = picName.trim() + ".png";
        }
        String filePath = System.getProperty("user.dir") + "/screenshot/" + picName;
		try {			
